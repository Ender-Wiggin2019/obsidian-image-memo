...
export class JournalingSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    let infoPathSetting = new Setting(containerEl)
      ...
      .setDesc("")
      .addButton((text) =>
        text.setButtonText("Save").onClick(() => {
          if (
            resourcesPathInput === "/" ||
            !(
    ...
    infoPathSetting.descEl.createDiv({
      text: "Specify an existing vault folder for the Journaling plugin to store image information/notes as markdown files.",
    });
    infoPathSetting.descEl.createDiv({
      text: "E.g. `Resources/Journaling`.",
      attr: { style: "color: indianred;" },
    });
    infoPathSetting.descEl.createDiv({
      text: "On first activation the default is unspecified. Thus the info functionality of the Main Journaling is diabled.",
    });
    infoPathSetting.descEl.createDiv({
      text: "Folder must already exist (plugin will not create it).",
      attr: { style: "font-weight: 900;" },
    });
    infoPathSetting.descEl.createDiv({
      text: "If a folder is not specified no Image Information notes are created (to be used in the main Journaling).",
    });

    new Setting(containerEl)
      .setName("Default Image Width")
      .setDesc(
        "Display default image width in `pixels`. integer, placeholder shows current value. (change this value to change the display in the main Journaling"
      )
      .addText((text) =>
        text
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Toggle this option to reverse the order in which the main Journaling displays images."
      )
      .addToggle((toggle) => {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        `The path from which to show images when the main Journaling is opened. 
